...
const skillSlice = createSlice({
  ...
  reducers: {
    ...
    postSkillRequest: (_state, action: PayloadAction<SkillStates>) => [
...
export const getSkillsAsync = () => async (
  dispatch: (arg0: { payload: Array<SkillState> }) => void
): Promise<void> => {
  axios
...
export const getSkillsByDeletedAsync = () => async (
  dispatch: (arg0: { payload: Array<SkillState> }) => void
): Promise<void> => {
  axios
...
export const postSkillAsync = (data: SkillStates) => async (
  dispatch: (arg0: { payload: SkillStates }) => void
): Promise<void> => {
  dispatch(requestLoading());
  axios
    .post(`/api/skill/post`, {
      params: { data: data.skills, query: "PostSkill" },
    })
    .then(() => {
